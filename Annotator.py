...
def annotator(image, room_img_res):
    ...
    annotator.sizelist = []
    annotator.doorlist = []
    annotator.openinglist = []
    annotator.splitconfirm = False
    annotator.unit_length = -1.0 
    input_value = ""
    ...
    def split_room_horizontally():
        #global annotator.mode, annotator.drawable, annotator.lastDraw
        if annotator.previous_button != SPLIT_ROOM_HORI and annotator.previous_button != -1:
            clean_up_button_color(annotator.previous_button)
        annotator.previous_button = SPLIT_ROOM_HORI
        annotator.drawable = 0
        annotator.lastDraw = 0
        annotator.mode = SPLIT_ROOM_HORI
    ...
    def split_room_vertically():
        #global annotator.mode, annotator.drawable, annotator.lastDraw
        if annotator.previous_button != SPLIT_ROOM_VERT and annotator.previous_button != -1:
            clean_up_button_color(annotator.previous_button)
        annotator.previous_button = SPLIT_ROOM_VERT
        annotator.drawable = 0
        annotator.lastDraw = 0
        annotator.mode = SPLIT_ROOM_VERT
    ...
    def save_to_file():
         ...
         for i in range(0, len(annotator.outlist)):
             c1 = [str(y) for y in annotator.outlist[i]]
             outputfile.write(" ".join(c1))
             outputfile.write(" : ")
             outputfile.write(annotator.textlist[i])
             outputfile.write(" : ")
             outputfile.write(str(annotator.sizelist[i]))
             outputfile.write("\n\n")
         for x in annotator.doorlist:
             c1 = [str(y) for y in x]
             outputfile.write(" ".join(c1))
             outputfile.write(" :  door\n\n")
         for x in annotator.openinglist:
             c1 = [str(y) for y in x]
             outputfile.write(" ".join(c1))
             outputfile.write(" :  opening\n\n")
         outputfile.close()

         #
         #  draw image
         #
         imgfilename = "Budas_result_img.jpg"
         image0 = image.copy() 
         c1 = 0
         c2 = 100
         c3 = 200
         i = 1
         for c in annotator.outlist: 
               p = []
               ...
               c3 = (c3 + 97) % 203
               i = i+1
         for y in annotator.doorlist:
            cv2.line(image0, (int(y[0]), int(y[1])), (int(y[2]), int(y[3])), (255, 255, 0), 5)
         for y in annotator.openinglist: 
            cv2.line(image0, (int(y[0]), int(y[1])), (int(y[2]), int(y[3])), (255, 255, 255), 5)
         cv2.imwrite(imgfilename, image0)

         #
         #  create graph
         #
         adjlist = checkadjacent(annotator.outlist, annotator.doorlist, annotator.openinglist)
         createGraph(adjlist, annotator.textlist, annotator.sizelist, "Budas_output.gv")
    ...
    def set_return_list():
        ...
        for x in olist:
            ...
            for z1 in z:
                if (z1[0:6] == "label_"):
                   annotator.textlist.append(z1[6:])
                   findText = True
                if (z1[0:5] == "size_"):
                   annotator.sizelist.append(z1[5:])
                   findSize = True
            if (not findText):
              annotator.textlist.append("")
            if (not findSize):   
              if (annotator.unit_length > 0):
                  area1 = calArea2(c) * annotator.unit_length
                  annotator.sizelist.append(str(area1))
              else:
                  annotator.sizelist.append(0)
        olist = canvas.find_withtag("door")
        for x in olist:
             c = canvas.coords(x)
             annotator.doorlist.append(c)
        olist = canvas.find_withtag("opening")
        for x in olist:
             c = canvas.coords(x)
             annotator.openinglist.append(c)
        window.destroy()
    ...
    def on_right_button_down(event):
        if (annotator.mode == SPLIT_ROOM_HORI and annotator.splitconfirm):
            canvas.delete("templinehori")
            annotator.split_button_2.config(bg=annotator.buttonBaseColor)
        elif (annotator.mode == SPLIT_ROOM_VERT and annotator.splitconfirm):
            canvas.delete("templinevert")
            annotator.split_button_3.config(bg=annotator.buttonBaseColor)
        annotator.msg.config(text="BUDAS")
        annotator.splitconfirm = False
        annotator.mode = 0
    ...
    def on_left_button_down(event):
        ...
        if annotator.mode == 3 or annotator.mode == -1:
            annotator.splitconfirm = False
            return
        if annotator.mode == 1 or  annotator.mode == 13 or annotator.mode == 14:
            annotator.splitconfirm = False
            ...
            annotator.drawable = 1
            return
        if annotator.mode == SPLIT_ROOM_VERT:
            if (annotator.splitconfirm):
               tlinecoord = canvas.coords("templinevert")
               _, _, first_index = on_the_edge(tlinecoord[0], tlinecoord[1], canvas.coords(annotator.cur_item))
               fixed_x, fixed_y, index = on_the_edge(tlinecoord[2], tlinecoord[3], canvas.coords(annotator.cur_item))
               if first_index <= index:
                   split_poly(annotator.cur_item,  tlinecoord[0], tlinecoord[1], tlinecoord[2], tlinecoord[3], first_index, index)
               else:
                   split_poly(annotator.cur_item,tlinecoord[2], tlinecoord[3], tlinecoord[0], tlinecoord[1], index, first_index)
               annotator.drawable = 0
               annotator.lastDraw = 0
               annotator.splitconfirm = False
               annotator.msg.config(text="BUDAS")
               ...
               annotator.index = index
               annotator.cur_item = event.widget.find_withtag("current")
               ppp = canvas.coords(annotator.cur_item)
               #print(ppp)
               seg = findintersectvertseg(ppp, [fixed_x, fixed_y])
               seg1 = [int(x) for x in seg]
               #print("seg 1 " + str(seg1))
               canvas.create_line(fixed_x, seg1[1], fixed_x, seg1[3], width=5, tags="templinevert")
               annotator.drawable = 1
               annotator.splitconfirm = True
               annotator.msg.config(text="Press left button to confirm, Press right button to cancel")
            return
        if  annotator.mode == SPLIT_ROOM_HORI:
            if (annotator.splitconfirm):
               tlinecoord = canvas.coords("templinehori")
               #print("tlinecoord " + str(tlinecoord))
               #print("cur_item " + str(canvas.coords(annotator.cur_item)))
               _, _, first_index = on_the_edge(tlinecoord[0], tlinecoord[1], canvas.coords(annotator.cur_item))
               fixed_x, fixed_y, index = on_the_edge(tlinecoord[2], tlinecoord[3], canvas.coords(annotator.cur_item))
               #print("first/index : " + str(first_index) + " " + str(index))
               if first_index <= index:
                   split_poly(annotator.cur_item,  tlinecoord[0], tlinecoord[1], tlinecoord[2], tlinecoord[3], first_index, index)
               else:
                   split_poly(annotator.cur_item,tlinecoord[2], tlinecoord[3], tlinecoord[0], tlinecoord[1], index, first_index)
               ...
               ppp = canvas.coords(annotator.cur_item)
               #print(ppp)
               seg = findintersecthoriseg(ppp, [fixed_x, fixed_y])
               seg1 = [int(x) for x in seg]
               canvas.create_line(seg1[0], fixed_y, seg1[2], fixed_y, width=5, tags="templinehori")
               annotator.cur_item = event.widget.find_withtag("current")
               annotator.drawable = 1
               annotator.splitconfirm = True
               annotator.msg.config(text="Press left button to confirm, Press right button to cancel")
    ...
    def on_left_button_move(event):
        ...
        cx = canvas.canvasx(event.x)
        cy = canvas.canvasy(event.y)
        if annotator.drawable == 0 or (annotator.mode != 1 and annotator.mode != 7 and annotator.mode != 12 and annotator.mode != 13 and annotator.mode != 14):
            return
    ...
    def on_left_button_up(event):
        ...
        if annotator.mode == 12: #enter size by draw lines
            ...
            annotator.mode = 0
            annotator.draw_size_button.config(bg=annotator.buttonBaseColor)
            annotator.unit_length = float(input_value) / math.sqrt((cx - annotator.X)*(cx - annotator.X) + (cy - annotator.Y)*(cy - annotator.Y))
            try:
                canvas.delete(annotator.lastDraw)
            except Exception as e:
                pass
            print("unit length:" + str(annotator.unit_length) + " ft")


        if annotator.mode == 1 or  annotator.mode == 13 or annotator.mode == 14:  # split room
            if (annotator.mode == 1):
                annotator.split_button_1.config(bg=annotator.buttonBaseColor)
            elif (annotator.mode == 13):
                annotator.insert_door_button.config(bg=annotator.buttonBaseColor)
            elif (annotator.mode == 14):
                annotator.insert_opening_button.config(bg=annotator.buttonBaseColor)
            ...
            if index == -1:
                ...
                return
            if annotator.mode == 13 or annotator.mode == 14:
                if index != first_index:
                    annotator.drawable = 0
                    canvas.delete(annotator.lastDraw)
                    annotator.lastDraw = 0
                    annotator.mode = 0
                    annotator.msg.config(text="Endpoint do not touch the same wall. Draw canceled")
                    return
                else:
                    annotator.drawable = 0
                    annotator.lastDraw = 0
                    if annotator.mode == 13:
                       canvas.create_line(annotator.X, annotator.Y, cx, cy, fill="#44FFA0", width=5, tag="door")
                    elif annotator.mode == 14:
                       canvas.create_line(annotator.X, annotator.Y, cx, cy, fill="#FF44A0", width=5, tag="opening")
                    annotator.msg.config(text="BUDAS")
                    return
    ...
    def on_seg(px, py, x1, y1, x2, y2):
        if (px - x1) * (y2 - y1) == (x2 - x1) * (py - y1) and min(x1, x2) <= px <= max(x1, x2) and min(y1, y2) <= py <= max(y1, y2):
            ...
            res =  False
        #print("         on_seg: ", px, py, x1, y1, x2, y2, res)
        return res
    ...
    def label_selected_room(item):
        ...
        #print(tags)
            ...
            #canvas.delete(canvas.find_withtag("TAG4"+str(item[0])))
        z = canvas.gettags(item)
        for z1 in z:
                if (z1[0:6] == "label_"):
                    canvas.dtag(item, z1)
        canvas.addtag_withtag("label_"+input_value, item)
    ...
    def size_of_selected_room(item):
        ...
        if len(entered_total_room_size) > 0:
            size_text = entered_total_room_size + "(ft^2)"
        elif len(entered_length) > 0 and len(entered_width) > 0:
            size_text = entered_length + " (ft) * " + entered_width + " (ft)"
        canvas.create_text(centerx, centery + 15, text=size_text, tag="SIZE4" + str(item[0]))
        canvas.addtag_withtag("size_"+size_text, item)
    ...
    def on_the_edge(px, py, coordinates):
        ...
        if (len(coordinates) >= 4):
            for i in range(0, len(dx)):
                #print("  i : ", i)
                for t in range(0, len(coordinates) - 3, 2):
                    #print("    t : ", t)
    ...
    def draw_door():
        ...
        annotator.drawable = 0
        annotator.mode = 13
        annotator.insert_door_button.config(bg="yellow")
        annotator.msg.config(text="Click on the wall where you want to insert a door")

    def draw_opening():
        if annotator.previous_button != 14 and annotator.previous_button != -1:
            clean_up_button_color(annotator.previous_button)
        annotator.previous_button = 14
        annotator.drawable = 0
        annotator.mode = 14
        annotator.insert_opening_button.config(bg="yellow")
        annotator.msg.config(text="Click on the wall where you want to insert a opening")
    ...
    annotator.insert_door_button = Button(window, text="Draw Door", command=draw_door)
    annotator.insert_door_button.grid(column=3, row=1)
    annotator.insert_opening_button = Button(window, text="Draw Opening", command=draw_opening)
    annotator.insert_opening_button.grid(column=4, row=1)
    ...
    #print("finally : ")
    #return room_img_res
    return [annotator.outlist, annotator.textlist, annotator.sizelist, annotator.doorlist, annotator.openinglist]
